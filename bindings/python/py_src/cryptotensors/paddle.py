...
import paddle
from cryptotensors import numpy


def save(tensors: Dict[str, paddle.Tensor], metadata: Optional[Dict[str, str]] = None, config: Optional[Dict[str, Any]] = None) -> bytes:
    """
...
def load(data: bytes, device: str = "cpu") -> Dict[str, paddle.Tensor]:
    ...
    ```python
    from cryptotensors.paddle import load
...
def load_file(filename: Union[str, os.PathLike], device="cpu") -> Dict[str, paddle.Tensor]:
    ...
    ```python
    from cryptotensors.paddle import load_file
