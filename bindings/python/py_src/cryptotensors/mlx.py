...
def save(tensors: Dict[str, mx.array], metadata: Optional[Dict[str, str]] = None, config: Optional[Dict[str, Any]] = None) -> bytes:
    """
...
def load(data: bytes) -> Dict[str, mx.array]:
    ...
    ```python
    from cryptotensors.mlx import load
...
def load_file(filename: Union[str, os.PathLike]) -> Dict[str, mx.array]:
    ...
    ```python
    from cryptotensors.flax import load_file
